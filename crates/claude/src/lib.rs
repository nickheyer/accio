...
use accio_provider::{parse_usage, read_json, write_atomic, Backend, Job, Outcome, Provider, Swap};
...
impl Claude {
    fn locate() -> Result<Self> {
        ...
        Ok(Claude { creds_path: claude_dir.join(".credentials.json"), claude_json_path })
    }
}
...
impl Backend for Claude {
    ...
    fn write_live(&self, contents: &BTreeMap<String, String>) -> Result<()> {
        ...
        if let Some(acct) = contents.get("oauth_account") {
        ...
        Box::new(move || {
            let mut creds: Value =
                match files.get("credentials").and_then(|s| serde_json::from_str(s).ok()) {
                    ...
                    None => {
                        return Outcome {
                            usage: Err("no credentials stored".to_string()),
                            state: None,
                        }
                    }
                };
